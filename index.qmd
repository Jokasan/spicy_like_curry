...
Next, we will filter the play-by-play data for all seasons less than or equal to 
2023 and match the opponent ids to the plays made by Stephen Curry. We will also
account for whether the play has no direct opponent, the resulting dataframe
is `base_staph_pbp`, which will be the foundation for modelling:

```{r}
#| eval: false
nba_pbp %>%  
  filter(season <= 2023 &
           athlete_id_1 == 3975 & # Filter Curry's plays
           shooting_play == TRUE & # Make sure all are shooting plays
